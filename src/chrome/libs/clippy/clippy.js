...
clippy.Agent.prototype = {
    ...
    speak:function (text, char, hold) {
        this._addToQueue(function (complete) {
            ...
            this._balloon.speak(complete, text, char, hold);
        }, this);
    },
...
clippy.Balloon.prototype = {
    ...
    speak:function (complete, text, char, hold) {
        this._hidden = false;
        this.show();
        var c = this._content;
        // set height to auto
        ...
        this._sayWords(text, char, hold, complete);
    },
    ...
    _sayWords:function (text, char, hold, complete) {
        this._active = true;
        this._hold = hold;
...
clippy.load._loadScript = function (src) {
	src = src.replace("//s3.amazonaws.com/clippy.js/Agents/", chrome.runtime.getURL("libs/clippy/agents/"));
	
	/*
	*** Need to load a diff way so that it runs as a content script ***
